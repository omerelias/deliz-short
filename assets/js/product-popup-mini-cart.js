...
(function () {
    ...
    function getFloatCartItemRow(el) {

        return el && el.closest ? el.closest('.ed-float-cart__item') : null;

    }



    function hideFloatCartRemoveConfirm(row) {
        ...
        const overlay = row.querySelector('.ed-float-cart__remove-confirm');
        ...
        row.classList.remove('is-remove-confirming');

    }



    function showFloatCartRemoveConfirm(row) {

        if (!row || !row.closest('#ed-float-cart')) {

            return;

        }

        document.querySelectorAll('#ed-float-cart .ed-float-cart__item.is-remove-confirming').forEach((r) => {

            if (r !== row) {

                hideFloatCartRemoveConfirm(r);
        ...
        const yesBtn = overlay.querySelector('.ed-float-cart__remove-confirm-yes');
     ...
     * כפתור × / כן / לא על שכבת "האם להסר?"

     */

    function handleFloatCartRemoveConfirmClick(e) {

        if (!e.target.closest('#ed-float-cart')) {
        ...
        const yesBtn = e.target.closest('.ed-float-cart__remove-confirm-yes');

        const noBtn = e.target.closest('.ed-float-cart__remove-confirm-no');

        const removeBtn = e.target.closest('.ed-float-cart__remove');
        ...
        if (yesBtn) {
            ...
            const row = getFloatCartItemRow(yesBtn);

            const key = row && row.dataset.cartItemKey;

            if (key) {

                hideFloatCartRemoveConfirm(row);
        ...
        if (noBtn) {
            ...
            hideFloatCartRemoveConfirm(getFloatCartItemRow(noBtn));
        ...
        if (removeBtn) {
            ...
            showFloatCartRemoveConfirm(getFloatCartItemRow(removeBtn));
    ...
    function seedFloatCartCommittedQty() {

        document.querySelectorAll('#ed-float-cart .ed-float-cart__qty-input').forEach((inp) => {

            inp.dataset.edCommittedQty = inp.value;
    ...
    function handleMiniCartQuantityClick(e) {
        ...
        if (action === 'increase') {
            ...
            if (candidate < minNum - eps || candidate <= 0) {
                ...
                logFloatCartQty('qty-btn', valueBeforeClick, 'לחיצה − → אישור הסרה', '(שכבה)', {
                ...
                showFloatCartRemoveConfirm(btn.closest('.ed-float-cart__item'));
    ...
    function handleMiniCartQuantityChange(e) {
        ...
        // Explicit 0 (or negative): שכבת אישור (לא מסירים מיד)

        if (isFinite(parsed) && parsed <= 0) {

            clearTimeout(input._updateTimeout);

            const revert = input.dataset.edCommittedQty || input._edQtyAtFocus || String(minNum);

            input.value = revert;

            logFloatCartQty('qty-input', valueBeforeInput, 'הקלדה ≤0 → אישור הסרה', input.value, {
            ...
            showFloatCartRemoveConfirm(getFloatCartItemRow(input));
            ...
            if (fragments && typeof fragments === 'object') {
                ...
                if (!isRemoval) {
                    ...
                    if (inpAfter) {
                        ...
                        inpAfter.dataset.edCommittedQty = inpAfter.value;
    ...
    async function handleMiniCartEditClick(e) {
        ...
        hideFloatCartRemoveConfirm(getFloatCartItemRow(editBtn));
    ...
    if (document.readyState === 'loading') {
        ...
        document.addEventListener('DOMContentLoaded', seedFloatCartCommittedQty);
    ...
    document.addEventListener('focusin', function (e) {
        ...
        const inp = e.target.closest && e.target.closest('.ed-float-cart__qty-input');

        if (!inp || !inp.closest('#ed-float-cart')) {

            return;

        }

        inp._edQtyAtFocus = inp.value;
    ...
    window.EDProductPopupMiniCart = {
        ...
        handleFloatCartRemoveConfirmClick,
