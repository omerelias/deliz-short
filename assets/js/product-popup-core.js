...
(function () {
    ...
    /**
     * Server rendered the front-page shop shell (mp_cat / mp_product) — not WC single product.
     */
    function isVirtualShopAppShell() {
        const b = document.body;
        return b.classList.contains('ed-virtual-shop-app') ||
    ...
    async function checkUrlForProduct() {
        ...
        if (catProductMatch) {
            // URL bar shows /cat/.../product/.../ but document is still WC single-product (e.g. old replaceState).
            if (!isVirtualShopAppShell() && document.body.classList.contains('single-product')) {
            ...
            try {
                ...
                if (response.ok) { 
                    ...
                    if (products.length) {
                        ...
                        // Keep /cat/{cat}/product/{slug}/ in the address bar (canonical, shareable URL).
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (products.length) {
                        ...
                        // Full navigation to front-page shell — replaceState left us on WC single-product DOM.
                        if (categorySlug) {
                            const canonSlug = products[0].slug || productSlug;
                            const virtualUrl = window.location.origin + '/cat/' + categorySlug + '/product/' + canonSlug + '/';
                            window.location.replace(virtualUrl);
                            return;
                        }
    ...
    function closePopup() {
        ...
        // After close: keep /cat/.../product/.../ in the bar; only normalize legacy /product/.../ URLs.
        if (pathname.match(/\/product\//) && !/^\/cat\/[^/]+\/product\/[^/]+\/?$/.test(pathname)) {
            const catMatch = pathname.match(/^\/cat\/([^/]+)\//);
            if (catMatch) {
