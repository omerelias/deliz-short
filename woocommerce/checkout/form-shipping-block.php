<?php
/**
 * Shipping checkout block (same markup as in form-shipping.php).
 * Loaded on checkout via form-shipping.php and in SMS wizard (non-checkout) for identical UI/behavior.
 *
...
<div class="checkout-block checkout-block--shipping is-closed <?php echo $local_pickup_chosen ? 'hidden' : ''; ?>" data-block="shipping" aria-expanded="false" data-popup-id="checkout-block-popup--shipping">
	<div class="checkout-block__summary-row">
		...
		if ( $address_house_num ) {
			$address_parts[] = $address_house_num;
		}
		if ( $cleaned_address_1 && ! $address_street ) {
			$address_parts[] = $cleaned_address_1; // Fallback if no street
		}
		if ( $address_city ) {
			$address_parts[] = $address_city;
		}
		...
		if ( $first_line ) {
			echo $first_line; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( $date_time_info ) {
			echo $date_time_info; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	...
	<div class="checkout-block-popup" id="checkout-block-popup--shipping" aria-hidden="true">
		...
		<div class="diff-address-shipping <?php echo $local_pickup_chosen ? 'hidden' : ''; ?>">
			...
			<h3>
				<label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox">
					...
					<input type="hidden" class="hidden shipping-to-other-address" id="shipping-to-other-address" name="oc_theme_ship_to_different_address" value="<?php echo esc_attr( $is_shipping_to_other_address ); ?>"  />
				</label>
			</h3>
		</div>
		...
		<div class="woocommerce-billing-fields__field-wrapper woocommerce-billing-fields-part-2 billing-fields-shipping-data-1 <?php echo $local_pickup_chosen ? 'hidden' : ''; ?>">
			...
			foreach ( $fields as $key => $field ) {
				if ( in_array( $key, $ar_billing_fields_first, true ) ) {
