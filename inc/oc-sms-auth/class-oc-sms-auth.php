...
class OC_SMS_Auth {
    ...
    private function init_hooks() {
        ...
        add_action('wp_ajax_oc_send_auth_sms', array($this, 'ajax_send_auth_sms'));

        add_action('wp_ajax_nopriv_oc_verify_sms_code', array($this, 'ajax_verify_sms_code'));
        add_action('wp_ajax_oc_verify_sms_code', array($this, 'ajax_verify_sms_code'));
    ...
    public function ajax_send_auth_sms() {
        ...
        if ( empty( $users ) ) {
            ...
            if ( ! $this->settings['allow_registration'] ) {
                wp_send_json_error(
                    array(
                        'message' => __( 'מספר הטלפון לא רשום במערכת', 'oc-main-theme' ),
                    )
                );
            }
        }
