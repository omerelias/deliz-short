...
// ✅ REST API endpoint for product popup data
add_action('rest_api_init', function () {
  register_rest_route('ed/v1', '/product-popup', [
    ...
    'callback' => 'ed_rest_get_product_popup_data',
  ]);
});

function ed_rest_get_product_popup_data(\WP_REST_Request $req) {
...
// Add customer note to cart item data (for popup)
add_filter('woocommerce_add_cart_item_data', 'ed_add_product_note_to_cart_item_data', 20, 2);
function ed_add_product_note_to_cart_item_data($cart_item_data, $product_id) {
