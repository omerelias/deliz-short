...
if (!function_exists('deliz_short_ocwsu_meta_weight_to_grams')) {
  /**
   * OCWSU _ocwsu_min_weight / _ocwsu_weight_step are entered in product weight units (kg or g).
   *
   * @param float|int|string $value Raw meta value.
   * @param string           $product_weight_units Post meta _ocwsu_product_weight_units.
   */
  function deliz_short_ocwsu_meta_weight_to_grams($value, $product_weight_units) {
    $v = floatval($value);
    if ($v <= 0) {
      return 0.0;
    }
    return deliz_short_ocwsu_product_weight_is_grams($product_weight_units) ? $v : ($v * 1000.0);
  }
}

if (!function_exists('deliz_short_ocwsu_format_fixed_unit_weight_display')) {
  /**
   * תצוגת משקל יחידה קבועה (OCWSU): מתחת ל־1 ק"ג — גרמים (700 גרם); מ־1 ק"ג ומעלה — ק"ג (2.5 ק"ג, לא 2500 גרם).
   * תומך גם ביחידת מוצר "גרם" בשדה (_ocwsu_unit_weight = 700).
   *
   * @param float|int|string $raw_unit_weight    ערך _ocwsu_unit_weight כפי שהוזן במטא.
   * @param string           $product_weight_units _ocwsu_product_weight_units.
   * @return string למשל "700 גרם" או "2.5 ק"ג", או מחרוזת ריקה.
   */
  function deliz_short_ocwsu_format_fixed_unit_weight_display($raw_unit_weight, $product_weight_units) {
    if (!function_exists('deliz_short_ocwsu_meta_weight_to_grams') || !function_exists('deliz_short_format_ocwsu_cart_weight_display_value')) {
      return '';
    }
    $raw = floatval($raw_unit_weight);
    if ($raw <= 0) {
      return '';
    }
    $kg = deliz_short_ocwsu_meta_weight_to_grams($raw, $product_weight_units) / 1000.0;
    if ($kg <= 0) {
      return '';
    }
    if ($kg < 1) {
      $g = deliz_short_format_ocwsu_cart_weight_display_value($kg * 1000, true);
      return sprintf('%s %s', $g, __('גרם', 'deliz-short'));
    }
    $kgs = deliz_short_format_ocwsu_cart_weight_display_value($kg, false);
    return sprintf('%s %s', $kgs, __('ק"ג', 'deliz-short'));
  }
}
...
class ED_Product_Popup {
      ...
      if (file_exists($template_file)) {
        ...
        foreach (array_reverse($cart->get_cart(), true) as $cart_item_key => $cart_item) {
          ...
          $min_w_numeric = ($min_w_meta_oc !== '' && is_numeric($min_w_meta_oc) && floatval($min_w_meta_oc) > 0)
              ...
              : 0.0;
          if ($min_w_numeric <= 0 && $sold_by_weight) {
            $min_w_numeric = 1.0;
          }
          $ocwsu_min_g = ($min_w_numeric > 0)
              ? deliz_short_ocwsu_meta_weight_to_grams($min_w_numeric, $product_weight_units)
              : 0.0;
          if ($ocwsu_min_g <= 0 && $sold_by_weight) {
          ...
          $ocwsu_step_kg_str = ($weight_step !== 'any' && is_numeric($weight_step))
              ...
              : 'any';
          $ocwsu_step_grams_str = ($weight_step !== 'any' && is_numeric($weight_step))
          ...
          if ($weighable) {
            ...
            // גם יחידות וגם משקל במוצר: לא להציג שורת ~משקל יחידה (כמו floating-mini-cart.php)
            ...
            if ( ! $ocwsu_skip_tilde_unit_line && $unit_weight_type_oc === 'fixed' && $sold_by_units && function_exists( 'deliz_short_ocwsu_format_fixed_unit_weight_display' ) ) {
              ...
              if ( ! $ocwsu_skip_unit_weight_meta_fallback && $ocwsu_one_item_kg <= 0 && ( $sold_by_units || $sold_by_weight ) ) {
                $unit_w_meta = get_post_meta($product_id, '_ocwsu_unit_weight', true);
                $unit_w_raw = ($unit_w_meta !== '' && is_numeric($unit_w_meta)) ? floatval($unit_w_meta) : 0.0;
                if ($unit_w_raw > 0) {
  ...
  public static function get_product_popup_data(\WP_REST_Request $req) {
    ...
    if ($weighable && $sold_by_units) {
      if ($unit_weight_type === 'fixed' && $unit_weight && ! $sold_by_weight) {
        // תצוגה אחידה: ~700 גרם / 2.5 ק"ג (לא 0.7 ק"ג גולמי). לא כשגם "נמכר לפי משקל" מסומן.
        $data['ocwsu']['average_weight'] = null;
  ...
  public static function enqueue_assets() {
    ...
    // 9. Mini cart (depends on state, core, quantity — edit-from-cart calls setQuantityMode)
    wp_enqueue_script(
      'deliz-short-product-popup-mini-cart',
      $base_path . 'product-popup-mini-cart.js',
      ['deliz-short-product-popup-state', 'deliz-short-product-popup-quantity', 'deliz-short-product-popup-core'],
      $version,
      true
    );
    ...
    wp_localize_script('deliz-short-product-popup-core', 'ED_POPUP_CONFIG', [
      ...
      'i18nFloatCart' => [
        ...
        'weightKg' => __('ק"ג', 'deliz-short'),
        'weightGram' => __('גרם', 'deliz-short'),
      ],
    ]);
  }
  ...
  public static function ajax_update_cart() {
    ...
    // Quantity 0 or below: remove line (minus button / user cleared qty)
    if ($quantity <= 0) {
      if ($cart_item) {
